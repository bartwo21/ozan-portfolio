...
metadata:
  ...
  updated: 2025-10-20
  python-tools: .claude/skills/content-creator/scripts/brand_voice_analyzer.py, .claude/skills/content-creator/scripts/seo_optimizer.py
...
1. Run `.claude/skills/content-creator/scripts/brand_voice_analyzer.py` on existing content to establish baseline
2. Review `.claude/skills/content-creator/references/brand_guidelines.md` to select voice attributes
3. Apply chosen voice consistently across all content

### For Blog Content Creation
1. Choose template from `.claude/skills/content-creator/references/content_frameworks.md`
2. Research keywords for topic
3. Write content following template structure
4. Run `.claude/skills/content-creator/scripts/seo_optimizer.py [file] [primary-keyword]` to optimize
5. Apply recommendations before publishing

### For Social Media Content
1. Review platform best practices in `.claude/skills/content-creator/references/social_media_optimization.md`
2. Use appropriate template from `.claude/skills/content-creator/references/content_frameworks.md`
3. Optimize based on platform-specific guidelines
4. Schedule using `.claude/skills/content-creator/assets/content_calendar_template.md`
...
1. **Analyze Existing Content** (if available)
   ```bash
   python .claude/skills/content-creator/scripts/brand_voice_analyzer.py existing_content.txt
   ```
   
2. **Define Voice Attributes**
   - Review brand personality archetypes in `.claude/skills/content-creator/references/brand_guidelines.md`
...
2. **Content Structure**
   - Use blog template from `.claude/skills/content-creator/references/content_frameworks.md`
   - Include keyword in title, first paragraph, and 2-3 H2s
   - Aim for 1,500-2,500 words for comprehensive coverage

3. **Optimization Check**
   ```bash
   python .claude/skills/content-creator/scripts/seo_optimizer.py blog_post.md "primary keyword" "secondary,keywords,list"
   ```
...
1. **Platform Selection**
   ...
   - Review platform-specific guidelines in `.claude/skills/content-creator/references/social_media_optimization.md`

2. **Content Adaptation**
   - Start with blog post or core message
   - Use repurposing matrix from `.claude/skills/content-creator/references/content_frameworks.md`
...
1. **Monthly Planning**
   - Copy `.claude/skills/content-creator/assets/content_calendar_template.md`
...
**Usage**: `python .claude/skills/content-creator/scripts/brand_voice_analyzer.py <file> [json|text]`
...
**Usage**: `python .claude/skills/content-creator/scripts/seo_optimizer.py <file> [primary_keyword] [secondary_keywords]`
...
```bash
# Analyze brand voice
python .claude/skills/content-creator/scripts/brand_voice_analyzer.py content.txt

# Optimize for SEO
python .claude/skills/content-creator/scripts/seo_optimizer.py article.md "main keyword"

# Check content against brand guidelines
grep -f .claude/skills/content-creator/references/brand_guidelines.md content.txt

# Create monthly calendar
cp .claude/skills/content-creator/assets/content_calendar_template.md this_month_calendar.md
```
